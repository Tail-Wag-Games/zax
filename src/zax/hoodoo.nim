when defined(windows):
  proc alignedAlloc*(size, alignment: uint): pointer {.sideeffect, importc:"_aligned_malloc", header:"<malloc.h>".}
  proc alignedFree*[T](p: ptr T) {.sideeffect, importc:"_aligned_free",
                                    header:"<malloc.h>".}
elif defined(macosx):
  proc posixMemalign(mem: var pointer, alignment, size: int){.importc: "posix_memalign", header:"<stdlib.h>".}
  proc alignedAlloc*(size, alignment: uint): pointer {.inline.} =
    posix_memalign(result, int(alignment), int(size))
  proc alignedFree*(p: pointer) {.importc: "free", header: "<stdlib.h>".}
...
template toIndex*(id: untyped): untyped =
  (id - 1)

template toId*(idx: untyped): untyped =
  (idx + 1)
