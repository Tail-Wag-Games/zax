import std/[hashes, locks, tables],
       api, handles, hoodoo, jobs, mstreams, zapis
...
proc onReadAsset(path: cstring; mem: ptr MemBlock;
  ...
  block outer:
    ...
    copyMem(buff, addr(res.path[0]), 256)
    buff += 256
...
proc createNewAsset(path: cstring; params: pointer; asset: Asset; name: cstring;
  ...
  var paramsId = 0'u32
  if paramsSize > 0:
    paramsId = toId(len(assetMgr.paramsBuff)).uint32
...
proc loadHashed(name: cstring; path: cstring; params: pointer;
  ...
  block outer:
    if isNil(path):
      ...
      echo("failed loading asset: invalid asset path")
      result.id = 0
      break outer
...
proc init*() =
  ...
  ctx.assetHandles = createHandlePool(256)
